...
const VideoDetail = ({ video, promises }) => {
  console.log('어떤 값이 들어오나요', video);
  console.log('어떤 값이 들어오나요', video.viewCount);
  ...
  if (video.viewCount < 1000) {
      ...
      result = `${num.toFixed(1)}회`;
  } else if (video.viewCount < 100000) {
      const num = video.viewCount / 10000;
      result = `${num.toFixed(1)}만회`;
  } else if (video.viewCount < 100000000) {
      result = `${Math.floor(video.viewCount / 10000)}만회`;
  } else {
      result = `${Math.floor(video.viewCount / 100000000)}억회`;
  }
      ...
      <div className='video_boxer'>
        ...
        <div className='tit_box'>
          <h1 className='video_tit'>
            ...
            <p className='video_mini mini_l'>조회수 {result} <span className='circle_txt'>•</span> {result2}</p>
          </h1>
        ...
        <div className='channel_box'>
          <div className='channel_tit'>
            ...
            <div className='channel_inner'>
              ...
              <p className='video_mini'>구독자 {video.subscriberCount}</p>
